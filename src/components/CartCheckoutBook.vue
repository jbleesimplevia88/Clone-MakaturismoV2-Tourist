          ...
          <div v-for="(hotel, index) in hotels" :key="index"
            ...
            <div class="border border-gray-400 rounded-xl p-5 ml-5 w-[90%]  h-fit">
              <div class="lg:flex items-center mb-5">
                <img :src="hotel.image" class="w-[100%] lg:w-[40%] h-[6rem] rounded-lg">
                <div class="ml-1 flex flex-col">
                  <div>
                    <p class="font-bold">{{ hotel.name }}</p>
                  </div>
                  <div>
                    <p class="mb-5 mt-2 text-gray-400">{{ hotel.type }}</p>
                  </div>
                  <div class="flex">
                    <p class="font-semibold text-sm mt-0">{{ hotel.rating }} Ratings</p>
                    <p class="text-sm text-gray-400">({{ hotel.reviews }} Review)</p>
                  </div>
                </div>
              </div>
              ...
              <div class="items-center mb-5">
                <div v-for="(item, index) in hotel.items" :key="index" class="flex justify-between mb-2 w-[100%]">
  ...
  computed: {
    totalPrice() {
      ...
      for (let item of this.hotels) {
        for (let hotel of item.items) {
          // Use hotel.price instead of shop.price
          total += parseFloat(hotel.price.replace("₱", "").trim());
        }
      }
      console.log("Total Price:", total.toFixed(2));
      return "₱ " + total.toFixed(2); // Format the total price
    },
