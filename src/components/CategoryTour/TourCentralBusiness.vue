                        ...
                        <div class="flex justify-between">
                            ...
                            <router-link to="/checkouttour">
                                <div class="w-[100%] px-2 mt-5">
                                    <button class="text-white flex justify-center mx-auto bg-blue-600 rounded-lg p-4 w-[100%]">Reserve</button>
                                </div>
                            </router-link>
                        </div>
                    </div>
        ...
        <div class="my-4 lg:p-0 lg:w-[75%]">
            <div class="relative mx-6 px-3 lg:px-32 pt-5">
                ...
                <div class="flex flex-row">
                    ...
                    <p class=" text-lg md:text-black text-left pl-2.5 pb-5 pt-3">{{ storedetails.address }}
                    </p>
                </div>
                <div class="flex flex-row">
                    <div class="pt-1">
                    ...
                    <div class="flex flex-col pl-2.5">
                        <p class="text-lg md:text-black text-left">{{ storedetails.startsat }} to {{ storedetails.endsat }}</p>
                        <p class="text-lg md:text-black text-left">{{ storedetails.actsched }}</p>
                    </div>
                </div>
                <div class="flex flex-row">
                    <div class="pt-0.25">
                    ...
                    <p class="text-lg md:text-black text-left pl-2.5 pb-5"> {{ storedetails.contactno }}</p>
                </div>
                        ...
                        <div class="lg:w-[75%] border border-gray-300 p-4 rounded-lg shadow mt-4">
                            <div class="mb-5">
                                <div class="flex flex-col m-2">
                                    ...
                                    <input type="date" v-model="selectedDate" class="border-2 border-black rounded-md p-2.5" :min="minDate" @change="checkDate">
                                </div>
                                <div class="flex flex-col m-2">
    ...
    <div class="relative flex flex-col">
        ...
        <div v-if="showSeeMoreButton || showSeeLessButton" class="flex items-center justify-center pr-[5rem] mt-5 mb-10">
            <div v-if="showSeeMoreButton">
                <div>
                    <div style="position: absolute; bottom: 0; left: 0; height: 50%; width: 100%; background: linear-gradient(to bottom, transparent 30%, #FFFFFF 79%, #FFFFFF 50%);">
                    </div>
                </div>
...
<script setup>
    ...
    const id = ref(route.params.id); // Automatically assign id from route params
    ...
    onMounted(async() => {
        console.log("Component mounted. Fetching details for ID:", id.value);
        if (id.value) {
            await fetchActivityDetails();
        }
    });
    async function fetchActivityDetails() {
        console.log(`Fetching activity details from API for ID: ${id.value}`);
        try {
            const response = await axios.get(`/viewactivityperid/${id.value}`);
            console.log("API response:", response.data);
            // Parse the 'peractivity' JSON string to convert it into an object
            if (response.data && response.data.peractivity) {
                storedetails.value = JSON.parse(response.data.peractivity);
                console.log("Parsed activity details:", storedetails.value);
            } else {
                console.error("No activity details found in the response");
            }
        } catch (error) {
