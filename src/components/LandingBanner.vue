<template>
  <div class="relative min-h-screen">
    ...
    <div v-if="isMobile">
      <!-- top gradient div -->
      <div style="position: absolute; top: 0; left: 0; height: 60%; width: 100%; background: linear-gradient(to top, transparent, #102E61 90%, #102E61 70%);"></div>

      <!-- Bottom gradient div -->
      <div style="position: absolute; bottom: 0; left: 0; height: 60%; width: 100%; background: linear-gradient(to bottom, transparent, #102E61 70%, #102E61 100%);"></div>
        <div class="absolute inset-0 flex flex-col justify-center px-10 pb-40 text-white top-40">
          ...
          <p class="mb-4 text-sm">Come and experience my Makati, our <a href="https://www.makati.gov.ph" target="_blank" class="text-blue-400 text-decoration-none">Makati</a></p>
        </div>
      ...
      <!-- Main nav -->
      <div class="fixed inset-x-0 bottom-0 flex items-end justify-center space-x-6 text-black bg-white" style="z-index: 2;">
        <div v-for="(item, index) in locations" :key="index" class="m-location-card" style="margin: 0; width: 100%; height: auto;"> <!-- Adjust height as needed -->
          <img :src="item.imgSrc" :alt="item.alt" class="h-auto p-1 mx-auto" style="width: 50%; height: auto;"> <!-- Adjust height as needed -->
    ...
    <div v-else>
      <!-- Left gradient div -->
    <div style="position: absolute; bottom: 0; left: 0; height: 100%; width: 40%; background: linear-gradient(to left, transparent, #102E61 100%"></div>

    <!-- Bottom gradient div -->
    <div style="position: absolute; bottom: 0; left: 0; height: 100%; width: 100%; background: linear-gradient(to bottom, transparent, #102E61 65%, #102E61 100%);"></div>
    ...
    <!-- Main nav -->
    <div class="absolute inset-x-0 bottom-0 flex items-end justify-center m-20 mb-10 space-x-6 text-black bg-white rounded-lg">
      <div v-for="(item, index) in locations" :key="index" :class="`location-card ${item.attrib}`" style="margin: 0;">
        <img :src="item.imgSrc" :alt="item.alt" class="w-5 pt-5 mx-auto" style="padding-left: 1rem; padding-right: 1rem;">
...
.location-card:hover {
  background-color: #008EE4; /* Set your desired background color on hover */
...
.location-card:hover h6 {
  color: white; /* Set your desired text color on hover */
  transition: filter 0.3s ease;
}
