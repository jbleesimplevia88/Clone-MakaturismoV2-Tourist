...
const displayToTour = async () => {
  try {
    ...
    console.log('Data fetched successfully:', response.data); // Log full response
    model.items = JSON.parse(response.data.viewactivities); // Parse the JSON string
    console.log('Parsed items:', model.items); // Log parsed items
    loading.value = false;
  } catch (err) {
...
const seeMore = (item) => {
  ...
  if (latitude !== null && longitude !== null) {
    router.push({
      name: 'central',
      params: { id: item.id },
      query: { latitude, longitude, item: item, name: item.activitytitle, id: item.id, imageList: item.activityphoto }
    });
  } else {
