<template>
          ...
          <div class="w-[100%] h-[35rem] z-10 space-y-2 overflow-y-auto hidden-scrollbar">
            <div v-for="(item, index) in filteredItems" :key="index"
              ...
              <div class="flex flex-col items-center mr-5">
                ...
                <button v-if="item.paymentstatus === 'Delivered'" @click="orderReceived(item)"
            ...
            <div class="flex flex-row">
              ...
              <div class="w-full">
                <div class="grid grid-cols-2">
                  <p class="h-[2rem] mt-3 text-md lg:text-lg">{{ selectedItem?.productname }}</p>
  ...
  class="px-4 py-2.5 p-2 bg-[#2969D6] font-bold text-white rounded-md text-center">Buy Again
</button>
  ...
  const success = await transactionStore.confirmOrderReceived(selectedItem.value.productid, selectedItem.value.orderid);
  if (success) {
    showOrderReceived.value = false;
